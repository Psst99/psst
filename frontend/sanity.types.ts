...
// Variable: upcomingWorkshopsQuery
// Query: *[_type == "workshop" && count(dates[@ >= now()]) > 0] | order(dates[0] asc){    _id,    title,    "slug": slug.current,    coverImage,    dates,    totalSpots,    "registrationsCount": count(*[_type == "workshopRegistration" && workshop._ref == ^._id && status == "approved"])  }
export type UpcomingWorkshopsQueryResult = Array<{
  _id: string
  title: string | null
  slug: string | null
  coverImage: {
